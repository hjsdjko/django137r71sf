				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('jiankangshuju','健康建议')" @click="jiankangjianyiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','此次健康申报已评测建议')" type="success">
...
import jiankangjianyiCrossAddOrUpdate from "../jiankangjianyi/add-or-update";
	export default {
		...
		methods: {
			...
			jiankangjianyiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
    ...
    init () {
          let urlshili = 'jiankangshuju/remind/shili/1';
		urlshili+=`?remindend=5.0`,
          ...
          this.$http({
            ...
            if (data && data.code == 0 && data.count>0 ) {
              this.$alert(`视力有${data.count}条记录到达预警`)
            }
          });
          let urlxueya = 'jiankangshuju/remind/xueya/1';

            	urlxueya+=`?remindstart=140`,
          ...
          this.$http({
            ...
            if (data && data.code == 0 && data.count>0 ) {
              this.$alert(`血压(mmHg)有${data.count}条记录到达预警`)
            }
          });
          let urlxuetang = 'jiankangshuju/remind/xuetang/1';

            	urlxuetang+=`?remindstart=7`,
          ...
          this.$http({
            ...
            if (data && data.code == 0 && data.count>0 ) {
              this.$alert(`血糖(mmol/L)有${data.count}条记录到达预警`)
            }
          });
    },
    search() {
