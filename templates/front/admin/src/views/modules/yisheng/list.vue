<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="content" label="状态">
						<template slot-scope="scope">
                            <el-switch
								...
								@change="(e)=>yishengstatusChange(e,scope.row)">
                            </el-switch>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yisheng','在线咨询')" @click="jiankangzixunCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								在线咨询
							</el-button>
							<el-button class="btn8" v-if="isAuth('yisheng','立即预约')" @click="yishengyuyueCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								立即预约
							</el-button>
		...
		<jiankangzixun-cross-add-or-update v-if="jiankangzixunCrossAddOrUpdateFlag" :parent="this" ref="jiankangzixunCrossaddOrUpdate"></jiankangzixun-cross-add-or-update>
		<yishengyuyue-cross-add-or-update v-if="yishengyuyueCrossAddOrUpdateFlag" :parent="this" ref="yishengyuyueCrossaddOrUpdate"></yishengyuyue-cross-add-or-update>
...
import jiankangzixunCrossAddOrUpdate from "../jiankangzixun/add-or-update";
import yishengyuyueCrossAddOrUpdate from "../yishengyuyue/add-or-update";
	export default {
		data() {
			return {
				keshiOptions: [],
				...
				jiankangzixunCrossAddOrUpdateFlag: false,
				yishengyuyueCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			jiankangzixunCrossAddOrUpdate,
			yishengyuyueCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			jiankangzixunCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiankangzixunCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiankangzixunCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiankangzixunCrossaddOrUpdate.init(row.id,type);
				});
			},
			yishengyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yishengyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yisheng');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yishengyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yishengyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
